...
export const CoreValueButton = ({ value, isSelected, onClick }) => (
  <button
    className={`p-1 g-white border-2 border-[#D7E0FF] shadow-[#D7E0FF_0px_0px_4px] hover:scale-[1.04] ${isSelected ? 'bg-[#D7E0FF]' : 'bg-white'}`}
...
export const ActionButton = ({ type, onClick, isEnabled }) => {
  ...
  switch (type) {
    case 'back':
      buttonText = 'Back';
      buttonClass = 'ml-[92px] py-2 px-9 rounded-3xl text-[#3A2A9B] font-semibold hover:scale-[1.04] border-2 border-[#3A2A9B] bg-white';
      break;
    case 'next':
      buttonText = 'Next';
      buttonClass = `mr-[92px] py-2 px-9 rounded-3xl text-white hover:scale-[1.04] float-right ${isEnabled ? 'bg-[#3A2A9B]' : 'bg-[#B9BBC3]'}`;
      break;
    case 'submit':
      buttonText = 'Submit';
      buttonClass = `mr-[92px] py-2 px-9 rounded-3xl text-white hover:scale-[1.04] float-right ${isEnabled ? 'bg-[#3A2A9B]' : 'bg-[#B9BBC3]'}`;
